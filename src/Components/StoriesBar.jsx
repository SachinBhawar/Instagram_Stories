...
function StoriesBar({ users, activeUserIdx, onUserClick }) {
    ...
    useEffect(() => {
        if (activeUserIdx !== null && activeUserIdx !== -1) {
            if (!seenUsers.includes(activeUserIdx)) {
                console.log("activeUserIdx:", activeUserIdx);
                setSeenUsers((prev) => [...prev, users[activeUserIdx].userId]);
    ...
    return (
        <div className="flex overflow-x-auto touch-auto py-4 bg-black/85 absolute top-0 left-0 w-screen z-30 gap-[18px] border-b border-white/10 no-scrollbar">
            {users.map((user, idx) => (
              ...
              ${seenUsers.includes(user.userId) ? "bg-gray-400" : "bg-orange-400"}
